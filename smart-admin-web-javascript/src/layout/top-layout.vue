...
<script setup>
  ...
  // 页面内容区域的高度（与实际头部/标签/面包屑高度精确对齐，避免覆盖导致点击偏移）
  const contentTop = computed(() => {
    const headerHeight = 48; // 顶部菜单实际高度
    const pageTagHeight = useAppConfigStore().$state.pageTagFlag ? 40 : 0;
    const breadCrumbHeight = useAppConfigStore().$state.breadCrumbFlag ? 40 : 0;
    return headerHeight + pageTagHeight + breadCrumbHeight + 'px';
  });
  ...
  const breadCrumbTop = computed(() => {
    ...
    const pageTagHeight = useAppConfigStore().$state.pageTagFlag ? 40 : 0;
    return headerHeight + pageTagHeight + 'px';
  });
