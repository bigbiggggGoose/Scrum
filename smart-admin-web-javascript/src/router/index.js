...
export function buildRoutes(menuRouterList) {
  ...
  // 统一按文件名匹配方法
  function matchByBaseName(modulesMap, baseName) {
    const name = baseName && baseName.endsWith('.vue') ? baseName : `${baseName}.vue`;
    const keys = Object.keys(modulesMap);
    let matchKey = keys.find((k) => k.endsWith('/' + name) && k.includes('/scrum/'));
    if (!matchKey) {
      matchKey = keys.find((k) => k.endsWith('/' + name));
    }
    return matchKey ? modulesMap[matchKey] : undefined;
  }

  // 组件路径回退解析：优先使用后端component路径，不存在则按文件名匹配
  function resolveComponent(modulesMap, componentPath) {
    if (!componentPath) {
      return undefined;
    }
    let normalized = componentPath.startsWith('/') ? componentPath : '/' + componentPath;
    if (!normalized.endsWith('.vue')) {
      normalized = `${normalized}.vue`;
    }
    const relativePath = `../views${normalized}`;
    let loader = modulesMap[relativePath];
    if (loader) {
      return loader;
    }
    try {
      const baseName = normalized.split('/').pop();
      return matchByBaseName(modulesMap, baseName);
    } catch (err) {
      console.warn('resolveComponent fallback error:', err);
    }
    return undefined;
  }

  // 基于路由path推断组件：/sys-user/list -> sys-user-list.vue
  function resolveByPath(modulesMap, routePath) {
    if (!routePath) {
      return undefined;
    }
    const parts = routePath.replace(/^\//, '').split('/');
    const baseName = `${parts.join('-')}.vue`;
    return matchByBaseName(modulesMap, baseName);
  }
  ...
  for (const e of menuList) {
    ...
    if (e.frameFlag) {
      ...
      let comp = resolveComponent(modules, componentPath);
      // 路径推断回退：component缺失或无法匹配时，基于path推断
      if (!comp) {
        comp = resolveByPath(modules, e.path);
      }
      // 最终兜底：404组件，避免出现无component警告
      route.component = comp || (() => import('../views/system/40X/404.vue'));
    }
