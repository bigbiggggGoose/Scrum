-- =============================================
-- Scrum Teams & Sprints seed (Drop FKs permanently)
-- 1) 永久去掉 scrum_sprints 的外键约束
-- 2) 更新产品（SmartAdmin 平台）项目级总人日与周期
-- 3) 插入团队与迭代数据（满足：每个迭代的人日与周期 < 项目级）
-- =============================================
...
-- ========== 6. 校验（可选） ==========
-- SmartAdmin 项目级信息
SELECT 'SmartAdmin 产品校验' AS info, total_man_day, plan_start_date, plan_end_date FROM `scrum_products` WHERE id = @product_smartadmin_id;
-- 迭代列表摘要
SELECT s.id, s.sprint_name, p.product_name, t.team_name, s.sprint_status, s.sprint_progress, s.total_man_day, s.consumed_hours, s.start_date, s.end_date
